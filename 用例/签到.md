...
C["<center>系统（System）查询由教员（Instructor）在机构（Oran）负责的<br>各课程（Course）、各课班（Class）、各授课安排（Schedule）上的参课（Attendance）信息"]
...
2. 营销页
	...
	* 展示信息
		...
		* 授课安排（Schedule）信息，包括上课时间（Start Time）、上课地点（Location）、上课教员（Instructor）、上课时长（Last Time）
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQwODI3MzgyNiwtOTMyNzg4MzcwLC02Mj
