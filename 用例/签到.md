...
TODO：未请假且未签到又实际上没到课的情况需要处理好，根据机构配置进入状态“未请假未参课自动扣课时”或“未请假未参课课时顺延”，状态的变化唯一由教员触发，系统不自动调整
TODO：请假的时效由机构配置，每门课程有自己的时效配置，在机构添加课程时系统引导配置，机构有默认的配置
...
准备签到码-->扫描签到码
扫描签到码-->列出负责的授课单元
列出负责的授课单元--使用默认时段-->展现授课单元下的参课信息
列出负责的授课单元--希望指定时段-->指定查询参课信息的时段
指定查询参课信息的时段-->列出负责的授课单元
展现授课单元下的参课信息-->选择参课信息
选择参课信息-->判断参课信息
判断参课信息--已签到-->选择参课信息
判断参课信息--未签到-->呈现签到按钮
...
4. 营销页
	...
	* 展示信息
		...
		* 授课安排（Schedule）信息，包括上课时间（Start Time）、上课地点（Location）、上课教员（Instructor）、上课时长（Last Time）
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMwMzgzMjIwMSwxMDA2OTY1NDUzLDM2ND
UxNTQ2MSwxMzAwNTk1NDgxLC0xOTY1Mjg1ODU2LC04MTAwNDMz
MjQsLTExMjgzMzE1MjQsLTEwODczODM5OTEsODkxMjY0NjIwLD
E3Mzk3NTE2MjUsLTIxNTExNzUwMiw3NjcyMDE3NywtMTk2ODUx
NTQzOSwxOTIwNjc3MDc0LC05MzI3ODgzNzAsLTYyNTQ2NzA3NS
wxMjI1NjcwNTg1LDIwMjE4NjQzNDIsMjAyMTg2NDM0MiwzNDk0
NDk3NThdfQ==
-->