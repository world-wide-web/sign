...
2. 机构签到页
	* 用户
		* 教员（Instructor）
	* 状态
		* a. 负责的课程（Course）、课班（Class）、授课安排（Schedule）
		- b. 参课（Attendance）信息浏览（默认态）
			* 功能
				* 快速浏览当前时间段内
...
4. 营销页
	...
	* 展示信息
		...
		* 授课安排（Schedule）信息，包括上课时间（Start Time）、上课地点（Location）、上课教员（Instructor）、上课时长（Last Time）
<!--stackedit_data:
eyJoaXN0b3J5IjpbODkxMjY0NjIwLDE3Mzk3NTE2MjUsLTIxNT
ExNzUwMiw3NjcyMDE3NywtMTk2ODUxNTQzOSwxOTIwNjc3MDc0
LC05MzI3ODgzNzAsLTYyNTQ2NzA3NSwxMjI1NjcwNTg1LDIwMj
