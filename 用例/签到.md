	...
	* 状态
		* a. 负责列表（默认态）
		...
		- b. 参课（Attendance）信息浏览
			* 功能
				* 快速浏览指定的课程（Course）、课班（Class）、授课安排（Schedule）在当前时间段内的参课（Attendance）信息，供点选触发代签到
...
4. 营销页
	...
	* 展示信息
		...
		* 授课安排（Schedule）信息，包括上课时间（Start Time）、上课地点（Location）、上课教员（Instructor）、上课时长（Last Time）
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjg4MDA2NzEyLC0xMTI4MzMxNTI0LC0xMD
g3MzgzOTkxLDg5MTI2NDYyMCwxNzM5NzUxNjI1LC0yMTUxMTc1
MDIsNzY3MjAxNzcsLTE5Njg1MTU0MzksMTkyMDY3NzA3NCwtOT
